...
def load_plugins():
    ...
    for filename in os.listdir(PLUGIN_DIR):
        if filename.endswith(".py") and not filename.startswith("temp_") and filename not in ["base.py", "manager.py", "__init__.py"]:
    ...
    try:
        ...
        for node in tree.body:
            if isinstance(node, ast.ClassDef):
                inherits = any((isinstance(b, ast.Name) and b.id == "MetadataPlugin") for b in node.bases)
                if inherits:
                    p_id = None
                    p_version = 1.0
